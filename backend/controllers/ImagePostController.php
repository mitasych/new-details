...
class ImagePostController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (!empty($_FILES['ImagePost']['name']['img'])) {
			$model->attributes=$_POST['ImagePost'];
// 			$imageName = substr(md5(uniqid()),0,24);
			$imageName = uniqid($_FILES['ImagePost']['name']['img']);
		
			$imageOb = CUploadedFile::getInstance($model,'img');
		
			$model->img = $imageName.'.'.$imageOb->extensionName;
		
			if($model->save()){
				$imageOb->saveAs(Yii::app()->params['imgUploadOrigin'].$model->main_image);
					
				$image = Yii::app()->image->load(Yii::app()->params['imgUploadOrigin'].$model->main_image);
...
// 				}
				...
				$this->imageResize($image, $model->img, $wm);
...
// 		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('ImagePost');
	...
	{
		$model=new ImagePost('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ImagePost']))
			$model->attributes=$_GET['ImagePost'];
	...
	{
		$model=ImagePost::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='image-post-form')
		{
